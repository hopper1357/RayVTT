...
// include: /tmp/tmpgznt7gbo.js
  ...
  (() => {
    ...
    async function loadPackage(metadata) {
      ...
      var PACKAGE_NAME = 'index_20250906233411.data';
      var REMOTE_PACKAGE_BASE = 'index_20250906233411.data';
...
Module['FS_createPath']("/", "assets", true, true);
      ...
      async function processPackageData(arrayBuffer) {
        ...
        // Reuse the bytearray from the XHR as the source for file reads.
          ...
          Module['removeRunDependency']('datafile_index_20250906233411.data');
      }
      Module['addRunDependency']('datafile_index_20250906233411.data');
    ...
    loadPackage({"files": [{"filename": "/assets/token.png", "start": 0, "end": 1677}], "remote_package_size": 1677});

  })();

// end include: /tmp/tmpgznt7gbo.js
// include: /tmp/tmpr0ueyyfd.js

    // All the pre-js content up to here must remain later on, we need to run
    // it.
    if ((typeof ENVIRONMENT_IS_WASM_WORKER != 'undefined' && ENVIRONMENT_IS_WASM_WORKER) || (typeof ENVIRONMENT_IS_PTHREAD != 'undefined' && ENVIRONMENT_IS_PTHREAD) || (typeof ENVIRONMENT_IS_AUDIO_WORKLET != 'undefined' && ENVIRONMENT_IS_AUDIO_WORKLET)) Module['preRun'] = [];
    var necessaryPreJSTasks = Module['preRun'].slice();
  // end include: /tmp/tmpr0ueyyfd.js
// include: /tmp/tmphoe7e8so.js

    if (!Module['preRun']) throw 'Module.preRun should exist because file support used it; did a pre-js delete it?';
    necessaryPreJSTasks.forEach((task) => {
      if (Module['preRun'].indexOf(task) < 0) throw 'All preRun tasks that exist before user pre-js code should remain after; did you replace Module or modify Module.preRun?';
    });
  // end include: /tmp/tmphoe7e8so.js
...
function findWasmBinary() {
    return locateFile('index_20250906233411.wasm');
}
...
var ASM_CONSTS = {
  101262: () => { if (document.fullscreenElement) return 1; },  
 101308: () => { return document.getElementById('canvas').width; },  
 101360: () => { return parseInt(document.getElementById('canvas').style.width); },  
 101428: () => { document.exitFullscreen(); },  
 101455: () => { setTimeout(function() { Module.requestFullscreen(false, false); }, 100); },  
 101528: () => { if (document.fullscreenElement) return 1; },  
 101574: () => { return document.getElementById('canvas').width; },  
 101626: () => { return screen.width; },  
 101651: () => { document.exitFullscreen(); },  
 101678: () => { setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { canvas.style.width="unset"; }, 100); }, 100); },  
 101811: () => { return window.innerWidth; },  
 101837: () => { return window.innerHeight; },  
 101864: () => { if (document.fullscreenElement) return 1; },  
 101910: () => { return document.getElementById('canvas').width; },  
 101962: () => { return parseInt(document.getElementById('canvas').style.width); },  
 102030: () => { if (document.fullscreenElement) return 1; },  
 102076: () => { return document.getElementById('canvas').width; },  
 102128: () => { return screen.width; },  
 102153: () => { return window.innerWidth; },  
 102179: () => { return window.innerHeight; },  
 102206: () => { if (document.fullscreenElement) return 1; },  
 102252: () => { return document.getElementById('canvas').width; },  
 102304: () => { return screen.width; },  
 102329: () => { document.exitFullscreen(); },  
 102356: () => { if (document.fullscreenElement) return 1; },  
 102402: () => { return document.getElementById('canvas').width; },  
 102454: () => { return parseInt(document.getElementById('canvas').style.width); },  
 102522: () => { document.exitFullscreen(); },  
 102549: ($0) => { document.getElementById('canvas').style.opacity = $0; },  
 102607: () => { return screen.width; },  
 102632: () => { return screen.height; },  
 102658: () => { return window.screenX; },  
 102685: () => { return window.screenY; },  
 102712: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 102765: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 102836: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 102893: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 103149: ($0) => { document.getElementById('canvas').style.cursor = UTF8ToString($0); },  
 103220: () => { if (document.fullscreenElement) return 1; },  
 103266: () => { return window.innerWidth; },  
 103292: () => { return window.innerHeight; },  
 103319: () => { if (document.pointerLockElement) return 1; }
};
function js_websocket_init_internal(url_cstr) { var url = UTF8ToString(url_cstr); var reconnectDelay = 1000; var heartbeatInterval; var lastPongTime; if (!localStorage.rayvttClientId) { localStorage.rayvttClientId = crypto.randomUUID(); console.log("Generated new client ID:", localStorage.rayvttClientId); } function setupWebSocket() { var ws = new WebSocket(url); ws.onopen = function () { console.log("WebSocket connected"); reconnectDelay = 1000; lastPongTime = Date.now(); heartbeatInterval = setInterval(function () { if (ws.readyState === WebSocket.OPEN) { ws.send(JSON.stringify({ type: "ping" })); if (Date.now() - lastPongTime > 10000) { console.warn("Pong timeout — closing WebSocket"); ws.close(); } } }, 5000); ws.send(JSON.stringify({ type: "reconnect_request", client_id: localStorage.rayvttClientId })); }; ws.onclose = function () { console.warn("WebSocket closed. Reconnecting in " + reconnectDelay + "ms..."); clearInterval(heartbeatInterval); setTimeout(setupWebSocket, reconnectDelay); reconnectDelay = Math.min(reconnectDelay * 2, 10000); }; ws.onerror = function (event) { console.error("WebSocket error:", event); }; ws.onmessage = function (event) { var data = event.data; try { var msg = JSON.parse(data); var sender_id_ptr = 0; var room_id_ptr = 0; var user_id_ptr = 0; if (msg.sender_id) { sender_id_ptr = Module.allocateUTF8(msg.sender_id); } if (msg.roomId) { room_id_ptr = Module.allocateUTF8(msg.roomId); } if (msg.userId) { user_id_ptr = Module.allocateUTF8(msg.userId); } if (msg.type === "init_state") { console.log("Received init_state"); if (msg.client_id) { localStorage.rayvttClientId = msg.client_id; var client_id_ptr = Module.allocateUTF8(msg.client_id); _network_set_client_id(client_id_ptr); Module._free(client_id_ptr); } for (var id in msg.tokens) { var tokenData = msg.tokens[id]; _network_on_update_token(0, parseInt(tokenData.id), tokenData.x, tokenData.y); } _network_on_ready(); } else if (msg.type === "update_token") { _network_on_update_token(sender_id_ptr, parseInt(msg.id), msg.x, msg.y); } else if (msg.type === "dice_roll") { var message_ptr = Module.allocateUTF8(msg.message); _network_on_add_dice_roll_message(sender_id_ptr, message_ptr); Module._free(message_ptr); } else if (msg.type === "pong") { lastPongTime = Date.now(); } else if (msg.type === "room_joined") { _network_on_room_joined(room_id_ptr); } else if (msg.type === "room_left") { _network_on_room_left(); } else if (msg.type === "user_joined") { _network_on_user_joined_room(user_id_ptr); } else if (msg.type === "user_left") { _network_on_user_left_room(user_id_ptr); } if (sender_id_ptr) { Module._free(sender_id_ptr); } if (room_id_ptr) { Module._free(room_id_ptr); } if (user_id_ptr) { Module._free(user_id_ptr); } } catch (e) { console.error("Failed to handle WebSocket message:", e, data); } }; window.rayvttWebSocket = ws; } setupWebSocket(); }
function js_websocket_send_internal(message_cstr) { if (window.rayvttWebSocket && window.rayvttWebSocket.readyState === WebSocket.OPEN) { window.rayvttWebSocket.send(UTF8ToString(message_cstr)); } else { console.warn("WebSocket not open. Message not sent: " + UTF8ToString(message_cstr)); } }
function js_websocket_close_internal() { if (window.rayvttWebSocket) { window.rayvttWebSocket.close(); } }

// Imports from the Wasm binary.
var _network_on_update_token = Module['_network_on_update_token'] = makeInvalidEarlyAccess('_network_on_update_token');
var _network_on_add_dice_roll_message = Module['_network_on_add_dice_roll_message'] = makeInvalidEarlyAccess('_network_on_add_dice_roll_message');
var _network_on_room_joined = Module['_network_on_room_joined'] = makeInvalidEarlyAccess('_network_on_room_joined');
var _network_on_room_left = Module['_network_on_room_left'] = makeInvalidEarlyAccess('_network_on_room_left');
var _network_on_user_joined_room = Module['_network_on_user_joined_room'] = makeInvalidEarlyAccess('_network_on_user_joined_room');
var _network_on_user_left_room = Module['_network_on_user_left_room'] = makeInvalidEarlyAccess('_network_on_user_left_room');
var _network_on_ready = Module['_network_on_ready'] = makeInvalidEarlyAccess('_network_on_ready');
var _main = makeInvalidEarlyAccess('_main');
var _network_set_client_id = Module['_network_set_client_id'] = makeInvalidEarlyAccess('_network_set_client_id');
...
function assignWasmExports(wasmExports) {
  Module['_network_on_update_token'] = _network_on_update_token = createExportWrapper('network_on_update_token', 4);
  Module['_network_on_add_dice_roll_message'] = _network_on_add_dice_roll_message = createExportWrapper('network_on_add_dice_roll_message', 2);
  Module['_network_on_room_joined'] = _network_on_room_joined = createExportWrapper('network_on_room_joined', 1);
  Module['_network_on_room_left'] = _network_on_room_left = createExportWrapper('network_on_room_left', 0);
  Module['_network_on_user_joined_room'] = _network_on_user_joined_room = createExportWrapper('network_on_user_joined_room', 1);
  Module['_network_on_user_left_room'] = _network_on_user_left_room = createExportWrapper('network_on_user_left_room', 1);
  Module['_network_on_ready'] = _network_on_ready = createExportWrapper('network_on_ready', 0);
  _main = createExportWrapper('main', 2);
  Module['_network_set_client_id'] = _network_set_client_id = createExportWrapper('network_set_client_id', 1);
...
var wasmImports = {
  ...
  /** @export */
  js_websocket_init_internal,
  /** @export */
  js_websocket_send_internal
};
