...
// end include: /tmp/tmp207sltbv.js
// include: /tmp/tmpbjtmatcf.js

    // All the pre-js content up to here must remain later on, we need to run
    // it.
    if ((typeof ENVIRONMENT_IS_WASM_WORKER != 'undefined' && ENVIRONMENT_IS_WASM_WORKER) || (typeof ENVIRONMENT_IS_PTHREAD != 'undefined' && ENVIRONMENT_IS_PTHREAD) || (typeof ENVIRONMENT_IS_AUDIO_WORKLET != 'undefined' && ENVIRONMENT_IS_AUDIO_WORKLET)) Module['preRun'] = [];
    var necessaryPreJSTasks = Module['preRun'].slice();
  // end include: /tmp/tmpbjtmatcf.js
// include: /tmp/tmpq1tkg9ro.js

    if (!Module['preRun']) throw 'Module.preRun should exist because file support used it; did a pre-js delete it?';
    necessaryPreJSTasks.forEach((task) => {
      if (Module['preRun'].indexOf(task) < 0) throw 'All preRun tasks that exist before user pre-js code should remain after; did you replace Module or modify Module.preRun?';
    });
  // end include: /tmp/tmpq1tkg9ro.js
...
var ASM_CONSTS = {
  100150: () => { if (document.fullscreenElement) return 1; },  
 100196: () => { return document.getElementById('canvas').width; },  
 100248: () => { return parseInt(document.getElementById('canvas').style.width); },  
 100316: () => { document.exitFullscreen(); },  
 100343: () => { setTimeout(function() { Module.requestFullscreen(false, false); }, 100); },  
 100416: () => { if (document.fullscreenElement) return 1; },  
 100462: () => { return document.getElementById('canvas').width; },  
 100514: () => { return screen.width; },  
 100539: () => { document.exitFullscreen(); },  
 100566: () => { setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { canvas.style.width="unset"; }, 100); }, 100); },  
 100699: () => { return window.innerWidth; },  
 100725: () => { return window.innerHeight; },  
 100752: () => { if (document.fullscreenElement) return 1; },  
 100798: () => { return document.getElementById('canvas').width; },  
 100850: () => { return parseInt(document.getElementById('canvas').style.width); },  
 100918: () => { if (document.fullscreenElement) return 1; },  
 100964: () => { return document.getElementById('canvas').width; },  
 101016: () => { return screen.width; },  
 101041: () => { return window.innerWidth; },  
 101067: () => { return window.innerHeight; },  
 101094: () => { if (document.fullscreenElement) return 1; },  
 101140: () => { return document.getElementById('canvas').width; },  
 101192: () => { return screen.width; },  
 101217: () => { document.exitFullscreen(); },  
 101244: () => { if (document.fullscreenElement) return 1; },  
 101290: () => { return document.getElementById('canvas').width; },  
 101342: () => { return parseInt(document.getElementById('canvas').style.width); },  
 101410: () => { document.exitFullscreen(); },  
 101437: ($0) => { document.getElementById('canvas').style.opacity = $0; },  
 101495: () => { return screen.width; },  
 101520: () => { return screen.height; },  
 101546: () => { return window.screenX; },  
 101573: () => { return window.screenY; },  
 101600: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 101653: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 101724: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 101781: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 102037: ($0) => { document.getElementById('canvas').style.cursor = UTF8ToString($0); },  
 102108: () => { if (document.fullscreenElement) return 1; },  
 102154: () => { return window.innerWidth; },  
 102180: () => { return window.innerHeight; },  
 102207: () => { if (document.pointerLockElement) return 1; }
};
function js_websocket_init_internal(url_cstr) { var url = UTF8ToString(url_cstr); var reconnectDelay = 1000; var heartbeatInterval; var lastPongTime; if (!localStorage.rayvttClientId) { localStorage.rayvttClientId = crypto.randomUUID(); console.log("Generated new client ID:", localStorage.rayvttClientId); } function setupWebSocket() { var ws = new WebSocket(url); ws.onopen = function () { console.log("WebSocket connected"); reconnectDelay = 1000; lastPongTime = Date.now(); heartbeatInterval = setInterval(function () { if (ws.readyState === WebSocket.OPEN) { ws.send(JSON.stringify({ type: "ping" })); if (Date.now() - lastPongTime > 10000) { console.warn("Pong timeout — closing WebSocket"); ws.close(); } } }, 5000); ws.send(JSON.stringify({ type: "reconnect_request", client_id: localStorage.rayvttClientId })); }; ws.onclose = function () { console.warn("WebSocket closed. Reconnecting in " + reconnectDelay + "ms..."); clearInterval(heartbeatInterval); setTimeout(setupWebSocket, reconnectDelay); reconnectDelay = Math.min(reconnectDelay * 2, 10000); }; ws.onerror = function (event) { console.error("WebSocket error:", event); }; ws.onmessage = function (event) { var data = event.data; try { var msg = JSON.parse(data); var sender_id_ptr = 0; if (msg.sender_id) { sender_id_ptr = Module.allocateUTF8(msg.sender_id); } if (msg.type === "init_state") { console.log("Received init_state"); if (msg.client_id) { localStorage.rayvttClientId = msg.client_id; var client_id_ptr = Module.allocateUTF8(msg.client_id); _network_set_client_id(client_id_ptr); Module._free(client_id_ptr); } for (var id in msg.tokens) { var tokenData = msg.tokens[id]; _network_on_update_token(0, parseInt(tokenData.id), tokenData.x, tokenData.y); } } else if (msg.type === "update_token") { _network_on_update_token(sender_id_ptr, parseInt(msg.id), msg.x, msg.y); } else if (msg.type === "dice_roll") { var message_ptr = Module.allocateUTF8(msg.message); _network_on_add_dice_roll_message(sender_id_ptr, message_ptr); Module._free(message_ptr); } else if (msg.type === "pong") { lastPongTime = Date.now(); } if (sender_id_ptr) { Module._free(sender_id_ptr); } } catch (e) { console.error("Failed to handle WebSocket message:", e, data); } }; window.rayvttWebSocket = ws; } setupWebSocket(); }
